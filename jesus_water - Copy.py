...
class Scene01_Titre(Scene):
    def construct(self):
        ...
        title = Tex(r"\textbf{Marcher sur l'eau}", font_size=50, color=AUBERGINE).to_edge(UP, buff=0.5)

        subtitle = TLines(
            r"\'Equations et fondements",
            r"de la physique",
            font_size=40, color=SOFT_BLACK, buff=0.15,
        ).next_to(title, DOWN, buff=0.4)

        sep = Line(LEFT * 1.8, RIGHT * 1.8, color=GOLD, stroke_width=1.5)
        sep.next_to(subtitle, DOWN, buff=0.3)

        author = Tex(r"Terre Math\'ematiques", font_size=30, color=AUBERGINE).next_to(sep, DOWN, buff=0.3)
...
class Scene05_DoOuVientF(Scene):
    def construct(self):
        ...
        header = make_header("D'où vient F ~ ρ A v² ?")
        self.play(Write(header), run_time=0.8)

        # ── Étape 1 : volume balayé ──
        step1 = Text("① Volume balayé en dt", font_size=14, color=AUBERGINE, weight=BOLD).move_to(UP * 2.7)
        ...
        self.play(FadeIn(foot), FadeIn(a_lab), run_time=0.4)
        self.play(FadeIn(cyl), GrowArrow(v_arrow), FadeIn(v_lab), run_time=0.6)

        eq1 = MathTex(r"\mathrm{d}V = A \cdot v\,\mathrm{d}t", font_size=22, color=AUBERGINE).move_to(UP * 0.5)
        self.play(Write(eq1), run_time=0.8)
        self.wait(1)

        self.play(FadeOut(step1), FadeOut(eq1), run_time=0.3)

        # ── Étape 2 : masse déplacée ──
        step2 = Text("② Masse d'eau déplacée", font_size=14, color=AUBERGINE, weight=BOLD).move_to(UP * 2.7)
        self.play(FadeIn(step2), run_time=0.4)
        self.play(cyl.animate.set_fill(WATER_DARK, opacity=0.7), run_time=0.5)
        ...
        self.play(FadeOut(step2), FadeOut(eq2), run_time=0.3)

        # ── Étape 3 : variation de quantité de mouvement ──
        step3 = Text("③ Quantité de mouvement", font_size=14, color=AUBERGINE, weight=BOLD).move_to(UP * 2.7)
        ...
        self.play(GrowArrow(p_arrow), FadeIn(p_lab), run_time=0.5)
        ...
        self.play(FadeOut(step3), FadeOut(eq3), run_time=0.3)

        # ── Étape 4 : 2e loi de Newton ──
        step4 = Text("④ Deuxième loi de Newton", font_size=14, color=AUBERGINE, weight=BOLD).move_to(UP * 2.7)
        self.play(FadeIn(step4), run_time=0.4)

        eq4a = MathTex(
            r"F = \frac{\mathrm{d}p}{\mathrm{d}t}",
            font_size=24, color=AUBERGINE,
        ).move_to(UP * 0.6)
        self.play(Write(eq4a), run_time=0.8)
        ...
        self.play(eq4b.animate.move_to(UP * 1.5), run_time=0.6)

        # ── Insight : pourquoi v² ? ──
        insight_title = Text("Pourquoi v² ?", font_size=16, color=AUBERGINE, weight=BOLD).move_to(UP * 0.3)
        ...
        insight = Text(
            ...
            font_size=13, color=SOFT_BLACK, line_spacing=1.5,
        ).move_to(DOWN * 0.9)
...
class Scene14_CTA(Scene):
    def construct(self):
        ...
        name = Tex(r"\textbf{Terre Math\'ematiques}", font_size=38, color=AUBERGINE).move_to(UP * 1.2)
        ...
        cta = TLines(
            ...
            font_size=28, color=SOFT_BLACK, buff=0.12,
