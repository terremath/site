...
class Scene1_RobotHonnete(Scene):
    def construct(self):
        ...
        rects = []
        labels_text = [
            ("LOI 3 — Se Préserver",     LOI3_BLUE,  11.0),
            ("LOI 2 — Obéir",            LOI2_AMBER,  9.0),
            ("LOI 1 — Ne Pas Blesser",   LOI1_RED,    7.0),
        ]
        ...
        arrow = Arrow(
            start=pyramid.get_bottom() + RIGHT * 6.0,
            end=pyramid.get_top() + RIGHT * 6.0,
        ...
        ).to_edge(RIGHT, buff=0.4).shift(UP * 2.0)
        ...
        # ─── 1.4 Le Lagrangien ───
        lag_title = Text("Le Lagrangien traduit la hiérarchie", font_size=SZ_SUBTITLE, color=GOLD)
        lag_title.to_edge(UP, buff=1.5)
        ...
        ).next_to(lag_title, DOWN, buff=0.6)
        lagrangian[1].set_color(LOI3_BLUE)
        lagrangian[2].set_color(LOI1_RED)
        lagrangian[3].set_color(LOI2_AMBER)

        brace_loi3 = Brace(lagrangian[1], DOWN, color=LOI3_BLUE, buff=0.1)
        brace_loi1 = Brace(lagrangian[2], DOWN, color=LOI1_RED, buff=0.1)
        brace_loi2 = Brace(lagrangian[3], DOWN, color=LOI2_AMBER, buff=0.1)
        bl3 = Text("LOI 3", font_size=14, color=LOI3_BLUE).next_to(brace_loi3, DOWN, buff=0.05)
        bl1 = Text("LOI 1", font_size=14, color=LOI1_RED).next_to(brace_loi1, DOWN, buff=0.05)
        bl2 = Text("LOI 2", font_size=14, color=LOI2_AMBER).next_to(brace_loi2, DOWN, buff=0.05)
        ...
        ], width=8.5, font_size=SZ_CAPTION + 2, color=GOLD)
        lag_note.next_to(all_braces, DOWN, buff=0.6)
